...
if __name__ == "__main__":
    ...
    for m_index in range(len(models_list)):
      ...
      for t in range(times):
        model = None
        if model_now == 'DIN':
          model = DIN(feature_columns, sess_feature_list, att_weight_normalization=True, dnn_hidden_units=(512, 256), dnn_activation='relu',
                      att_hidden_size=(512, 256), l2_reg_dnn=1e-6, seed=t, l2_reg_embedding=1e-2).cuda()
        elif model_now == 'FM':
          model = DeepFM(linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 128), dnn_activation='relu', l2_reg_linear=1e-7, l2_reg_embedding=1e-2, l2_reg_dnn=1e-7, seed=t)
        elif model_now == 'AutoInt':
          model = AutoInt(linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 128), l2_reg_embedding=5e-3, l2_reg_dnn=1e-7, seed=t)
        elif model_now == 'xDeepFM':
          model = xDeepFM(linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(512, 256, 128), dnn_activation='relu', l2_reg_linear=1e-7, l2_reg_embedding=2e-2, l2_reg_dnn=1e-7, l2_reg_cin=1e-7, seed=t, gpus=['cuda:0', 'cuda:1'])
        elif model_now == 'DCN':
          model = DCN(linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 128), dnn_activation='relu', l2_reg_linear=1e-7, l2_reg_embedding=1e-2, l2_reg_dnn=1e-6, l2_reg_cross=1e-7, seed=t)
        elif model_now == 'FinalMLP':
          model = FinalMLP(dnn_feature_columns, mlp1_hidden_units = (400, 400, 400), mlp2_hidden_units=[800], num_heads=10, l2_reg_embedding=1e-2, l2_reg_dnn=1e-6, seed=t)
        elif model_now == 'FiGNN':
          model = FiGNN(dnn_feature_columns, gnn_layers=1, use_residual=True, use_gru=True, reuse_graph_layer=True, l2_reg_embedding=1e-3, l2_reg_dnn=1e-6, seed=t+1, gpus=['cuda:0', 'cuda:1'])
        ...
        #torch.save(model.state_dict(), 'DeepFM_Baseline_Taobao.pth')
